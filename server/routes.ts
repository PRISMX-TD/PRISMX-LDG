...
import { setupAuth, isAuthenticated, isAuthDisabled, SESSION_COOKIE } from "./neonAuth";
...
export async function registerRoutes(
  ...
  // ---- Session cookie helpers (httpOnly — page JS cannot read the token) ----
  const SESSION_TTL_MS = 7 * 24 * 60 * 60 * 1000; // keep in sync with authToken TTL
  ...
  // Auth middleware with fallback or explicit disable (dev-only backdoor)
  if (isAuthDisabled()) {
  ...
  app.post('/api/auth/register', async (req, res) => {
    try {
      ...
      // upsertUser already seeds default wallets/categories (initializeUserDefaults),
      // so no second explicit init call is needed.
      ...
      res.status(201).json({ userId: user.id, user: sanitizeUser(user) });
    } catch (err) {
  ...
  app.post('/api/auth/login', async (req, res) => {
    try {
      ...
      // Avoid user enumeration: return the same message whether the account is missing
      // or the password is wrong, and always spend roughly the same time (dummy scrypt
      // when there is no stored hash) to flatten the timing side channel.
      const GENERIC = "邮箱或密码错误";
      if (!user || !user.passwordHash) {
        try { verifyPassword(password, "scrypt:00:00"); } catch { /* equalize timing */ }
      ...
      const token = signToken(user.id);
      setSessionCookie(res, token);
      res.json({ userId: user.id, user: sanitizeUser(user) });
    } catch (err) {
  ...
  // ---- Auth: logout — clear the httpOnly session cookie -------------------
  ...
  app.post('/api/account/forgot-password', async (req, res) => {
    try {
      ...
      // Build reset URL from a server-configured base URL. SECURITY: never trust the
      // client-supplied Origin/Host headers here — an attacker could set them to point
      // the (legitimately emailed) reset link at their own domain and steal the token.
      const configuredBase = (process.env.PUBLIC_APP_URL || "").replace(/\/+$/, "");
      if (!configuredBase) {
        console.warn("[forgot-password] PUBLIC_APP_URL not set — falling back to Host header (set PUBLIC_APP_URL in production).");
      }
      const base = configuredBase || `https://${req.headers.host}`;
  ...
  app.patch('/api/wallets/:id', isAuthenticated, async (req: any, res) => {
    try {
      ...
      // Allow restoring an archived wallet via PATCH { isArchived: false }.
      if (req.body.isArchived !== undefined) {
        const archived = req.body.isArchived === true;
  ...
  app.post('/api/wallets/:id/archive', isAuthenticated, async (req: any, res) => {
    try {
      ...
      const archived = await storage.updateWallet(id, userId, {
        isArchived: true,
        archivedAt: new Date(),
        isDefault: false,
      } as any);
  ...
  // Delete wallet.
  // Default behaviour = ARCHIVE (soft delete): the wallet is hidden from active lists and
  // asset totals, but its transactions are kept intact and remain viewable in history.
  // Only ?deleteTransactions=true performs a hard delete (which cascade-removes the
  // wallet's transactions) — that's the explicit "delete everything" path.
  app.delete('/api/wallets/:id', isAuthenticated, async (req: any, res) => {
    try {
      ...
      // Only active (non-archived) wallets count toward the "last wallet" guard.
      const wallets = await storage.getWallets(userId);
      if (wallets.length <= 1) {
        return res.status(400).json({ message: "Cannot delete the last wallet" });
      }

      // If removing the default wallet, promote another active wallet to default.
      ...
      if (deleteTransactions) {
        // Hard delete: remove the wallet's transactions, then the wallet itself.
        await storage.deleteTransactionsByWallet(id, userId);
        const deleted = await storage.deleteWallet(id, userId);
        return deleted ? res.status(204).send() : res.status(500).json({ message: "Failed to delete wallet" });
      }

      // Soft delete: archive so transactions are preserved (FK cascade would otherwise
      // delete them). Also drop the default flag if it was set.
      const archived = await storage.updateWallet(id, userId, {
        isArchived: true,
        archivedAt: new Date(),
        isDefault: false,
      ...
      if (!archived) {
        return res.status(500).json({ message: "Failed to archive wallet" });
      }
  ...
  app.get('/api/transactions/export', isAuthenticated, async (req: any, res) => {
    try {
      ...
      // Escape each cell: double internal quotes AND neutralize spreadsheet formula
      // injection (values starting with = + - @ or a control char get a leading ').
  ...
  app.post('/api/transactions', isAuthenticated, async (req: any, res) => {
    try {
      ...
      // SECURITY/INTEGRITY: validate ownership of nested ids BEFORE touching balances, so a
      // foreign/invalid id can't cause a mid-operation FK failure that leaves a wallet
      // debited without a recorded transaction.
      if (body.categoryId != null) {
        const cat = await storage.getCategory(body.categoryId, userId);
        if (!cat) return res.status(400).json({ message: "Invalid category" });
      }
      if (body.subLedgerId != null) {
        const sl = await storage.getSubLedger(body.subLedgerId, userId);
        if (!sl) return res.status(400).json({ message: "Invalid sub-ledger" });
      }
      if (body.loanId != null) {
        const ln = await storage.getLoan(body.loanId, userId);
        if (!ln) return res.status(400).json({ message: "Invalid loan" });
      }
      ...
      // Collect balance changes and apply them together with the insert (atomic).
      const deltas: { walletId: number; delta: number }[] = [];
      ...
      if (transactionData.type === 'transfer') {
        ...
        deltas.push({ walletId: wallet.id, delta: -walletAmount });
        deltas.push({ walletId: toWallet.id, delta: destAmount });
      ...
      // Insert the transaction and apply the balance changes in one DB transaction.
      const transaction = await storage.createTransactionWithEffects(transactionData, deltas);
  ...
  app.patch('/api/transactions/:id', isAuthenticated, async (req: any, res) => {
    try {
      ...
      // Validate ownership of nested ids before any balance math (see create handler).
      if (body.categoryId != null) {
        const cat = await storage.getCategory(body.categoryId, userId);
        if (!cat) return res.status(400).json({ message: "Invalid category" });
      }
      ...
      // Compute the reversal of the existing transaction's effect. It is applied together
      // with the new effect + row update inside one DB transaction (no read-modify-write,
      // no partial state if something fails midway).
      const reversalDeltas: { walletId: number; delta: number }[] = [];
      {
        const oldAmount = parseFloat(existingTransaction.amount || "0");
        if (existingTransaction.type === 'expense') {
          reversalDeltas.push({ walletId: existingTransaction.walletId, delta: oldAmount });
          ...
          reversalDeltas.push({ walletId: existingTransaction.toWalletId, delta: -toAmount });
        }
      }
      ...
      // Compute the new transaction's effect on wallet balances.
      const applyDeltas: { walletId: number; delta: number }[] = [];
      if (transactionData.type === 'transfer') {
        if (!transactionData.toWalletId) {
          return res.status(400).json({ message: "Transfer requires destination wallet" });
        }
        ...
        if (isToWalletCrossCurrency) {
          ...
          destAmount = toWalletAmount;
          transactionData.toWalletAmount = destAmount.toFixed(2);
          // FIX (L7): persist the destination exchange rate on update too (create already did).
          if (toExchangeRate && toExchangeRate > 0) {
            transactionData.toExchangeRate = toExchangeRate.toFixed(6);
          }
        } else {
        ...
        applyDeltas.push({ walletId: wallet.id, delta: -walletAmount });
        applyDeltas.push({ walletId: toWallet.id, delta: destAmount });
      } else if (transactionData.type === 'expense') {
        applyDeltas.push({ walletId: wallet.id, delta: -walletAmount });
      } else if (transactionData.type === 'income') {
        applyDeltas.push({ walletId: wallet.id, delta: walletAmount });
      ...
      // Reverse old effect + update the row + apply new effect — atomically.
      const updatedTransaction = await storage.updateTransactionWithEffects(
        transactionId, userId, transactionData, reversalDeltas, applyDeltas,
      );
  ...
  app.delete('/api/transactions/:id', isAuthenticated, async (req: any, res) => {
    try {
      ...
      // Reverse this transaction's effect on wallet balances.
      const reversalDeltas: { walletId: number; delta: number }[] = [];
      const amount = parseFloat(existingTransaction.amount || "0");
      if (existingTransaction.type === 'expense') {
        reversalDeltas.push({ walletId: existingTransaction.walletId, delta: amount });
      } else if (existingTransaction.type === 'income') {
        reversalDeltas.push({ walletId: existingTransaction.walletId, delta: -amount });
      } else if (existingTransaction.type === 'transfer' && existingTransaction.toWalletId) {
        reversalDeltas.push({ walletId: existingTransaction.walletId, delta: amount });
      ...
      // Delete the row and reverse the balances atomically.
      const deleted = await storage.deleteTransactionWithEffects(transactionId, userId, reversalDeltas);
      if (!deleted) {
        return res.status(404).json({ message: "Transaction not found" });
      }
  ...
  app.patch('/api/loans/:id', isAuthenticated, async (req: any, res) => {
    try {
      ...
      // SECURITY: whitelist updatable fields. Never let the client mass-assign
      // userId/id/paidAmount (paidAmount is derived by recalculateLoanStatus).
      const updateData: any = {};
      const lb = req.body || {};
      if (lb.type !== undefined) updateData.type = lb.type;
      if (lb.person !== undefined) updateData.person = lb.person;
      if (lb.totalAmount !== undefined) updateData.totalAmount = String(lb.totalAmount);
      if (lb.currency !== undefined) updateData.currency = lb.currency;
      if (lb.status !== undefined) updateData.status = lb.status;
      if (lb.description !== undefined) updateData.description = lb.description;
      if (lb.startDate !== undefined) updateData.startDate = lb.startDate ? new Date(lb.startDate) : null;
      if (lb.dueDate !== undefined) updateData.dueDate = lb.dueDate ? new Date(lb.dueDate) : null;
      ...
      if (updateData.status === 'bad_debt' && existingLoan.status !== 'bad_debt') {
        ...
        // Remove the write-off transaction. Query by its tag directly instead of scanning
        // the latest 100 rows (which missed it once the user had >100 newer transactions).
        const marker = `bad_debt_writeoff:loan_${id}`;
        const matches = await storage.getTransactionsByTag(userId, marker);
        for (const txToDelete of matches) {
  ...
  app.post('/api/bill-reminders/:id/pay', isAuthenticated, async (req: any, res) => {
    try {
      ...
      // Create the expense transaction and debit the wallet atomically.
  ...
  app.patch('/api/user/currency-v2', isAuthenticated, async (req: any, res) => {
    try {
      ...
      if (autoRescale === true) {
        // Fetch the old->new rate. If we can't get a real rate, do NOT silently rescale by
        // 1 (that would corrupt every wallet's exchange rate) — fail loudly instead.
        ...
        try {
          ...
          if (r.ok) {
            ...
            if (typeof fetched === "number" && fetched > 0) rate = fetched;
          }
        } catch {}
        if (rate === null) {
          return res.status(400).json({ message: "无法获取汇率，暂无法自动换算，请稍后再试或手动调整钱包汇率" });
        }
        const ws = await db.select().from(walletsTable).where(eq(walletsTable.userId, userId));
        for (const w of ws) {
  ...
  const MAX_BATCH_IDS = 500; // guard against unbounded arrays (DoS via N sequential queries)
  app.post('/api/transactions/batch-delete', isAuthenticated, async (req: any, res) => {
    try {
      ...
      if (ids.length > MAX_BATCH_IDS) {
        return res.status(400).json({ message: `一次最多处理 ${MAX_BATCH_IDS} 条` });
      }
      let deleted = 0;
      for (const rawId of ids) {
        ...
        // Reverse the wallet effect, same as the single-delete path — applied atomically.
        const reversalDeltas: { walletId: number; delta: number }[] = [];
        const amount = parseFloat(existing.amount || "0");
        if (existing.type === 'expense') {
          reversalDeltas.push({ walletId: existing.walletId, delta: amount });
          ...
          reversalDeltas.push({ walletId: existing.toWalletId, delta: -toAmount });
        }
        const ok = await storage.deleteTransactionWithEffects(id, userId, reversalDeltas);
        if (ok) deleted++;
        if (existing.loanId) {
  ...
  app.post('/api/transactions/batch-categorize', isAuthenticated, async (req: any, res) => {
    try {
      ...
      if (cid !== null) {
        const cat = await storage.getCategory(cid, userId);
        if (!cat) return res.status(400).json({ message: "categoryId 无效" });
      }
      let updated = 0;
      for (const rawId of ids) {
        const id = parseInt(rawId);
