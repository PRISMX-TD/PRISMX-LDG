...
export default function Dashboard() {
  ...
  const streak = useMemo(() => {
    ...
    let n = 0;
    const d = new Date();
    // Today is still "in progress": if nothing's logged yet today, start the
    // count from yesterday so the existing streak isn't shown as 0 until the
    // user records today. Logging today then extends it (e.g. 1 → 2).
            ...
            <div className="relative grid grid-cols-1 md:grid-cols-12 gap-5 md:gap-6 min-h-[230px]">
              ...
              <div className="md:col-span-7 min-w-0 flex flex-col justify-between gap-4">
                ...
                <div className="flex items-baseline gap-1 leading-none">
                  ...
                  <span className="text-[32px] sm:text-[40px] md:text-[54px] lg:text-[64px] font-bold tracking-tight break-all"
                        style={{
                  ...
                  {!isPrivacyMode && (
                    <span className="text-[16px] sm:text-[20px] md:text-[24px] lg:text-[28px] font-semibold text-foreground/40">.{fmtDec(animTotal)}</span>
                  )}
                </div>
